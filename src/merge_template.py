...
class TemplateMerger:
    def __init__(self, base_path: str = "onboarding"):
        # Default base_path is relative to the backend folder when used from backend
    ...
    def import_project_files(self, project_name: str) -> Dict[str, Any]:
        """
    Import project definition files from a sibling `projects/{project_name}` under the same
    onboarding base_path and write a compatible `overrides.json` into the merger projects folder.
        ...
        Assumptions:
          - If multiple role files exist, the first is treated as the primary role for template selection.
            All role files will be included under `project_specific.roles` for reference.
                - The source folder is resolved as `self.base_path / 'projects' / project_name` so
                    placing files in `backend/onboarding/projects/{project_name}` (relative to backend) will be found
                    when the merger's base_path is `onboarding` (the backend default).
        """
        # Expect project sources to live under the same base_path/projects
        src_root = self.base_path / 'projects' / project_name
    ...
    # Load project overrides; if missing attempt to import from onboarding/projects under base_path
        override_file = self.projects_path / project_name / "overrides.json"
        overrides = self.load_json(override_file)

        if not overrides:
            print(f"overrides.json not found for '{project_name}', attempting to import from {self.base_path}/projects/{project_name}")
