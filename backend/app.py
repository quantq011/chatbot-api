...
# Initialize Template Merger (use backend-local onboarding folder)
# Default to backend/onboarding so the project no longer depends on top-level documents/
merger = TemplateMerger(base_path=str(Path(__file__).parent / "onboarding"))
...
async def merge_template(request: MergeRequest):
    ...
    try:
        ...
        # Compute output path using merger.projects_path to avoid top-level documents/ usage
        output_path = request.output_file or str(merger.projects_path / request.project_name / "merged_config.json")
...
async def get_project_config(project_name: str):
    ...
    """
    config_path = merger.projects_path / project_name / "merged_config.json"
